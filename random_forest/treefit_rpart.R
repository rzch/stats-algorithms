...
#This script learns a decision tree using the rpart package
...
#fit tree using information gain
treefit <- rpart(is_wta~w_ace+w_df+w_1stPct+w_1stPctWon+w_2ndPctWon+l_ace+l_df+l_1stPct+l_1stPctWon+l_2ndPctWon, 
                 data = df_train, method = "class", parms = list(split = "information"))
...
is_wta_pred = predict(treefit, df_test, type="class") #make predictions on training data
